...
estimators = [
    GPT2Estimator('gpt2-xl', device=torch.device(3)),
    GPTJEstimator('EleutherAI/gpt-j-6b', device=torch.device(3)),
    LLaMA2Estimator('meta-llama/Llama-2-13b-chat-hf', device=torch.device(2)),
    AlpacaEstimator('/path/to/alpaca-7b/', device=torch.device(1)),
    VicunaEstimator('lmsys/vicuna-13b-v1.5', device=torch.device(0)),
    GPT35TurboEstimator(os.environ['OPENAI_API_KEY']),
    GPT4TurboEstimator(os.environ['OPENAI_API_KEY'])
]
